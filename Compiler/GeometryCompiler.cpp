...
{
    void GeometryCompiler::LoadModels(const std::string& modelFile)
    {
    ...
    void GeometryCompiler::WriteToFile(std::unordered_map<std::string, std::unique_ptr<Mesh>>& In)
    {
        ...
        try
        {
            GeometryFile.open("Geometry.txt", std::fstream::in | std::fstream::out | std::fstream::binary | std::ios::trunc);
        }
    ...
    {
       LoadModels("..//Eclipse//src//Assets//ASSModels");
    }
    ...
    {
        GeometryFileWrite.open("CSC2134.txt", std::ios::out | std::ios::in);
        std::string eachline;

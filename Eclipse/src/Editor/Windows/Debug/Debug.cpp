...
namespace Eclipse
{
	...
	void DebugWindow::DrawImpl()
	{
		...
		if (ECGui::ButtonBool("test button for darren"))
		{
			ImGui::OpenPopup("test change");
		}
		if (ImGui::BeginPopup("test change"))
		{
			ImGui::SetScrollY(5);
			ChildSettings settings{ "test change", ImVec2{ 250,250 } };
			ECGui::DrawChildWindow<void()>(settings, std::bind(&DebugWindow::tempwindow, this));

			ImGui::EndPopup();
		}
	}
	void DebugWindow::tempwindow()
	{
		std::vector<std::string> testnames1 = { "darren","jian herng" };
		std::vector<std::string> testnames2 = { "ty","dawawagfawfaw" };
		static size_t test1index = 0;
		static size_t test2index = 0;
		ECGui::CreateComboList({ "testname1", }, testnames1, test1index);
		ECGui::InsertHorizontalLineSeperator();
		ECGui::CreateComboList({ "testname2" }, testnames2, test2index);
	}
}

...
namespace Eclipse
{
		...
		mViewportSize{ 0.0f, 0.0f }
	{
		...
		IsVisible = false;
		m_frameBuffer_RIGHT = engine->GraphicsManager.mRenderContext.GetFramebuffer(FrameBufferMode::SWITCHINGVIEWS_RIGHT);
	...
	void RightSwitchViewWindow::RenderSettingsHeader()
	{
		size_t projIndex = 0;
		...
		{
			if (IsWireframeMode)
				m_frameBuffer_RIGHT->SetRenderMode(FrameBuffer::RenderMode::Wireframe_Mode);
			else
				m_frameBuffer_RIGHT->SetRenderMode(FrameBuffer::RenderMode::Fill_Mode);
		}
	}

	void RightSwitchViewWindow::OnKeyPressedEvent()
	{
	...
	void RightSwitchViewWindow::OnGizmoUpdateEvent()
	{
	...
	void RightSwitchViewWindow::OnCameraZoomEvent()
	{
		ImGuiIO& io = ImGui::GetIO();
	...
	void RightSwitchViewWindow::OnCameraMoveEvent()
	{
		ImGuiIO& io = ImGui::GetIO();

...
namespace Eclipse
{
		...
		mViewportSize{ 0.0f, 0.0f }
	{
		...
		IsVisible = false;
		m_frameBuffer_BOTTOM = engine->GraphicsManager.mRenderContext.GetFramebuffer(FrameBufferMode::SWITCHINGVIEWS_BOTTOM);
	...
	void BottomSwitchViewWindow::RenderSettingsHeader()
	{
		size_t projIndex = 0;
		...
		{
			if (IsWireframeMode)
				m_frameBuffer_BOTTOM->SetRenderMode(FrameBuffer::RenderMode::Wireframe_Mode);
			else
				m_frameBuffer_BOTTOM->SetRenderMode(FrameBuffer::RenderMode::Fill_Mode);
		}
	}

	void BottomSwitchViewWindow::OnKeyPressedEvent()
	{
	...
	void BottomSwitchViewWindow::OnGizmoUpdateEvent()
	{
	...
	void BottomSwitchViewWindow::OnCameraZoomEvent()
	{
		ImGuiIO& io = ImGui::GetIO();
	...
	void BottomSwitchViewWindow::OnCameraMoveEvent()
	{
		ImGuiIO& io = ImGui::GetIO();

...
namespace Eclipse
{
		...
		mViewportSize{ 0.0f, 0.0f }
	{
		...
		IsVisible = false;
		m_frameBuffer_LEFT = engine->GraphicsManager.mRenderContext.GetFramebuffer(FrameBufferMode::SWITCHINGVIEWS_LEFT);
	...
	void LeftSwitchViewWindow::RenderSettingsHeader()
	{
		size_t projIndex = 0;
		...
		{
			if (IsWireframeMode)
				m_frameBuffer_LEFT->SetRenderMode(FrameBuffer::RenderMode::Wireframe_Mode);
			else
				m_frameBuffer_LEFT->SetRenderMode(FrameBuffer::RenderMode::Fill_Mode);
		}
	}

	void LeftSwitchViewWindow::OnKeyPressedEvent()
	{
	...
	void LeftSwitchViewWindow::OnGizmoUpdateEvent()
	{
	...
	void LeftSwitchViewWindow::OnCameraZoomEvent()
	{
		ImGuiIO& io = ImGui::GetIO();
	...
	void LeftSwitchViewWindow::OnCameraMoveEvent()
	{
		ImGuiIO& io = ImGui::GetIO();

...
#define EDITOR_LOG_INFO(...) { if(engine->GetEditorState()) { ENGINE_CORE_INFO(__VA_ARGS__); Eclipse::LoggerWindow::GetEditorLogger()->AddLogs(__VA_ARGS__); } }
...
#define EDITOR_LOG_WARN(...) { if(engine->GetEditorState()) { ENGINE_CORE_WARN(__VA_ARGS__); Eclipse::LoggerWindow::GetEditorLogger()->AddLogs(__VA_ARGS__); } }
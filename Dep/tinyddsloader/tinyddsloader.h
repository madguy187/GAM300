...
Result DDSFile::Load(std::vector<uint8_t>&& dds) {
    ...
    if (dxt10Header) {
        auto dxt10Header_ = reinterpret_cast<const HeaderDXT10*>(
            reinterpret_cast<const char*>(header) + sizeof(Header));

        m_arraySize = dxt10Header_->m_arraySize;
        if (m_arraySize == 0) {
            return Result::ErrorInvalidData;
        }

        switch (dxt10Header_->m_format) {
            case DXGIFormat::AI44:
            case DXGIFormat::IA44:
            case DXGIFormat::P8:
            case DXGIFormat::A8P8:
            ...
            default:
                if (GetBitsPerPixel(dxt10Header_->m_format) == 0) {
                    return Result::ErrorNotSupported;
                }
        }

        m_format = dxt10Header_->m_format;

        switch (dxt10Header_->m_resourceDimension) {
        ...
        m_texDim = dxt10Header_->m_resourceDimension;
    } else {
